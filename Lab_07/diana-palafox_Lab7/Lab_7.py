...
def Q1(_conn):
    ...
    # where arguments will be passed in this case there are none
    print("Q1:")
   
   # Where we are going to write the sql
   # Needs try: sql "xxxx"
    try:
        ...
        #we must put args but since there are none we leave [] empty    
        args = []
        ...
        #this will format our name for column 
        #{:>10s} we add an s because its a string 
        #spacing is calculated by the number of space right to left
        #for example wId highlight 1.res row of the names 
        # then from right to left dots will appear
        # then count dots and name wId total will be 10. 
        #this will format our name for column 
        #{:>10s} we add an s because its a string 
        b = '{:>10s} {:<1s} {:>45s} {:>10s} {:>10s}\n'.format("wId", "wName", "wCap","sId","nId")
        print(b)
        
        #this is to write therefor in we use w in the output
        #the we add with write and close 
        with open('output/1.out', 'w') as z:
            z.write(str(b))
            z.close()
        #
        rows = cur.fetchall()
        
        #putting the stuff in rows
        for row in rows:
            l = '{:>20} {:<35} {:>15} {:>10} {:>10}\n'.format(row[0], row[1], row[2], row[3], row[4])
            print(l)
       
        # 'a' lets you output or i think "read"
        with open('output/1.out', 'a') as z:
            for row in rows:
                l = '{:10} {:<35} {:>15} {:>10} {:>10}\n'.format(row[0], row[1], row[2], row[3], row[4])
                z.write(str(l))
            z.close()
...
def Q2(_conn):
   ...
   # Needs try: sql "xxxx"
    ...
    try:
        sql ="""SELECT n_name as nation, count(w_nationkey) as numW, sum(w_capacity) as totCap
...
def Q3(_conn):
    ...
    try:
        sql ="""SELECT s_name, n_name, w_name
                ...
                WHERE w_suppkey = s_suppkey AND s_nationkey = n_nationkey AND w_name LIKE ? """
        
        # input the file inside 
        inPut = open('input/3.in', 'r')
        a = inPut.readline().replace('\n','')
        _JAP = a
        ...
        # then we are opening it 
        r = open('output/3.out', 'w')
...
def Q4(_conn):
    ...
    try:
        sql ="""SELECT w_name, w_capacity
                ...
                WHERE r_regionkey = n_regionkey AND n_nationkey = w_nationkey AND r_name = ? AND w_capacity > ?
        ...
        # b will also have the ablitiy to read 
        b = inPut.readline().replace('\n','')
        _wCap = b
        
        # creating argument for _rName, _wCaps 
        args = [_rName,_wCap]
...
def Q5(_conn):
    ...
    try:
        sql ="""SELECT r_name, sum(w_capacity)
                FROM nation S, supplier, warehouse, region, nation R
                WHERE S.n_nationkey = s_nationkey AND s_suppkey = w_suppkey AND S.n_name = ? AND w_nationkey = R.n_nationkey AND R.n_regionkey = r_regionkey
                GROUP BY r_regionkey"""
        
        # input the file inside
        inPut = open('input/5.in', 'r')
        d = inPut.readline().replace('\n','')
        _UNITEDSTATES = d

        # we are going to check the regions 
        reg = ['AFRICA', 'AMERICA', 'ASIA', 'EUROPE', 'MIDDLE EAST']
        args = [_UNITEDSTATES]
